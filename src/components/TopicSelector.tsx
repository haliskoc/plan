...
export const TopicSelector = React.memo(function TopicSelector() {
  ...
  return (
    <div className="flex flex-col h-full bg-neutral-950/90 border border-neutral-900 rounded-2xl p-4 sm:p-5 shadow-xl">
      <div className="mb-4">
        <h2 className="text-sm font-bold text-white uppercase tracking-wider mb-3">Konu Kütüphanesi</h2>
        <div className="grid grid-cols-2 p-1 bg-neutral-900 border border-neutral-800 rounded-xl mb-3">
          <button onClick={() => setSelectedExamType("TYT")} className={`py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${selectedExamType === "TYT" ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/10" : "text-neutral-400 hover:text-neutral-200"}`}>
            TYT Konuları
          </button>
          <button onClick={() => setSelectedExamType("AYT")} className={`py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${selectedExamType === "AYT" ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/10" : "text-neutral-400 hover:text-neutral-200"}`}>
            AYT Konuları
          </button>
        </div>
        ...
        {selectedExamType === "AYT" && (
          <div className="grid grid-cols-3 p-1 bg-neutral-900 border border-neutral-800 rounded-xl mb-3">
            <button onClick={() => setSelectedTrack("SAY")} className={`py-1 text-[11px] font-bold rounded-md transition-all cursor-pointer ${selectedTrack === "SAY" ? "bg-neutral-800 text-white shadow-xs" : "text-neutral-400 hover:text-neutral-200"}`}>
              Sayısal (SAY)
            </button>
            <button onClick={() => setSelectedTrack("EA")} className={`py-1 text-[11px] font-bold rounded-md transition-all cursor-pointer ${selectedTrack === "EA" ? "bg-neutral-800 text-white shadow-xs" : "text-neutral-400 hover:text-neutral-200"}`}>
              Eşit Ağırlık (EA)
            </button>
            <button onClick={() => setSelectedTrack("SOZ")} className={`py-1 text-[11px] font-bold rounded-md transition-all cursor-pointer ${selectedTrack === "SOZ" ? "bg-neutral-800 text-white shadow-xs" : "text-neutral-400 hover:text-neutral-200"}`}>
              Sözel (SÖZ)
            </button>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          <button onClick={() => setSelectedSubject("")} className={`px-2 py-1 rounded-md text-[10px] font-bold transition-all cursor-pointer ${!selectedSubject ? "bg-indigo-600 text-white" : "bg-neutral-900 text-neutral-400 hover:text-neutral-200"}`}>
            Tümü
          </button>
          {allSubjects.map((sub) => (
            <button key={sub} onClick={() => setSelectedSubject(selectedSubject === sub ? "" : sub)} className={`px-2 py-1 rounded-md text-[10px] font-bold transition-all cursor-pointer ${selectedSubject === sub ? "bg-indigo-600 text-white" : "bg-neutral-900 text-neutral-400 hover:text-neutral-200"}`}>
              {sub}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto pr-1 space-y-2 max-h-[calc(100vh-390px)] scrollbar-thin scrollbar-thumb-neutral-800 scrollbar-track-transparent">
        {Object.keys(groupedTopics).length === 0 ? (
          ...
          Object.keys(groupedTopics).sort().map((subjectName) => {
            ...
            return (
              <div key={subjectName} className="border border-neutral-900 rounded-xl bg-neutral-900/10 overflow-hidden">
                <button onClick={() => toggleSubject(subjectName)} className="w-full flex items-center justify-between px-4 py-3 bg-neutral-900/30 hover:bg-neutral-900/50 transition-colors text-left cursor-pointer">
                  <span className="text-xs font-bold text-neutral-300">{subjectName}</span>
                  <span className="flex items-center gap-2">
                    <span className="text-[10px] text-neutral-500 px-2 py-0.5 bg-neutral-900 rounded-full font-mono">{topicsInGroup.length}</span>
                ...
                {isExpanded && (
                  <div className="divide-y divide-neutral-900/50 bg-neutral-950/20 max-h-80 overflow-y-auto">
                    {topicsInGroup.map((topic) => {
                      ...
                      return (
                        <div key={topic.id} className="flex items-center justify-between px-4 py-2 hover:bg-neutral-900/20 group transition-all">
                          <div className="flex flex-col gap-0.5 pr-2">
                            <span className="text-xs text-neutral-300 group-hover:text-white transition-colors">{topic.name}</span>
                            {count > 0 && (
                              <span className="inline-flex items-center gap-0.5 text-[9px] font-semibold text-emerald-400 font-mono">
                                <Check className="w-2.5 h-2.5" />{count} kez eklendi
                              </span>
                            )}
                          </div>
                          <button onClick={() => handleAddTopic(topic.id)} className="p-1 rounded-md bg-neutral-900 hover:bg-indigo-600 text-neutral-400 hover:text-white border border-neutral-800 hover:border-indigo-500 cursor-pointer shadow-xs transition-all shrink-0" title="Tarihe Ekle">
